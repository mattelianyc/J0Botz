...
const keywords = [
  'apply', 'career', 'careers', 'job', 'jobs','recruiting', 'talent', 'work', 'join', 'employment', 'team', 'people', 'hr', 'hiring'
];
...
(async () => {
  ...
  let companyIndex = 0; // Track the current company index
  ...
  const sendEmail = async (mailOptions) => {
    ...
    try {
      ...
      if (error.code === 'ECONNRESET') {
        ...
        return false; // Indicate that we should retry sending the email
      }
...
<a href="https://www.mattelia.me">www.mattelia.me</a>`,
      ...
      while (!emailSent) {
        try {
          ...
          if (emailSent) {
            emailCount++; // Increment the email counter
            ...
            currentEmailIndex++;
            emailCount = 0; // Reset counter for the next email account
          ...
          // Check if the email count has reached 475
          if (emailCount >= 475) {
            console.log(`Reached 475 emails sent from ${emailCredentials[currentEmailIndex].user}. Switching to next email.`);
            currentEmailIndex++;
            emailCount = 0; // Reset counter for the next email account
            if (currentEmailIndex >= emailCredentials.length) {
              console.log('All email accounts have been exhausted.');
          ...
          if (error.code === 'EAUTH') {
            ...
            console.log('Connection reset. Reconnecting...');
            // Do not increment companyIndex, just retry sending the email
          } else {
    ...
    companyIndex++; // Move to the next company after processing all keywords
  }
